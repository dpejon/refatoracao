...
public class MeusPostDAO {
	...
	public List<Mural> carregarTodosPosts(int ID_Criador) {
		...
		Mural registrar = new Mural();
		registrar.setId_criador(ID_Criador);
		String sqlSelect = "SELECT ID, Titulo, Conteudo FROM post where ID_Criador = ?";
		// usando o try with resources do Java 7, que fecha o que abriu
		try (Connection conn = ConnectionFactory.obterConexao();
				PreparedStatement stm = conn.prepareStatement(sqlSelect);) {
			stm.setInt(1, registrar.getId_criador());
